...
class MemoryEncoder(nn.Module):
    ...
    def __init__(self, rnn_type, encoder_type,
        ...
        if self.self_attn:
            ...
            self.selfattn_ws = nn.Linear(hidden_size, hidden_size, bias=False) # W_s
            self.selfattn_wf = nn.Linear(hidden_size * 2, hidden_size)  # W_f
            self.selfattn_wg = nn.Linear(hidden_size * 2, hidden_size)  # W_g
    ...
    def forward(self, src, src_lengths,
        ...
        if self.self_attn:
            ...
            src_bank_t = src_bank.transpose(0, 1).contiguous() # (h_j)^p
            src_bank_all = self.selfattn_ws(src_bank_t.view(-1, src_h)).view(src_bs, src_maxlen, src_h) # (H^p)^T * W_s
            ...
            def sequence_mask2d(lengths, max_len=None):
                ...
                mask2d_inv = ~(mask1d.unsqueeze(-1)) * ~(mask1d.unsqueeze(1))
            ...
            src_score.masked_fill_(~(src_mask.cuda()), -float('inf'))
            src_weight = self.softmax(src_score)  # (a_p)^j
            # src_weight.masked_fill_(1 - src_mask.unsqueeze(-1).cuda(), 0.0)
            src_selfattn_s = torch.bmm(src_weight, src_bank_t) # (u_p)^j : self-matching representation
