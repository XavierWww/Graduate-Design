...
class Statistics(object):
    ...
    * elapsed time
    ...
    def __init__(self, loss=0, flow_loss=0, flow_history_loss=0, corefvocab_loss=0, corefattn_loss=0,
                 num_effective_coref=0, n_words=0, n_correct=0):
        # num_effective_coref: number of coreference examples in the current batch
    ...
    def output(self, step, num_steps, learning_rate, start):
        ...
        logger.info(
            ("Step %2d/%5d; acc: %6.2f; ppl: %5.2f; xent: %4.2f; " +
             "corefvocab xent: %4.2f; corefattn xent: %4.2f; " +
             "flow loss: %4.2f; " +
             "flow history loss: %4.2f;" +
             "lr: %7.5f; %3.0f/%3.0f tok/s; %6.0f sec")
            % (step, num_steps,
               self.accuracy(),
               self.ppl(),
               self.xent(),
               self.xent_corefvocab(),
               self.xent_corefattn(),
               self.flow_loss,
               self.flow_history_loss,
               learning_rate,
