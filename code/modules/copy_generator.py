...
class CopyGenerator(nn.Module):
    ...
    * :math:`p_{copy}` the probility of copying a particular word.
    ...
    def __init__(self, input_size, tgt_dict):
        ...
        self.linear = nn.Linear(input_size, len(tgt_dict), bias=True)
...
class CopyGeneratorLossCompute(nn.Module):
    ...
    def compute_loss(self, batch, output, attns, normalization):
        ...
        #print("lambda_lambda_coveragelambda_coveragelambda_coveragelambda_coverage")
        #TODO
        if self.lambda_coverage != 0.0:
            coverage_loss = self._compute_coverage_loss(attns.get("std"), attns.get("coverage_attn"))
            loss += coverage_loss
            #print("lambda_coverage")
