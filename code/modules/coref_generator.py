...
class CorefGeneratorLossCompute(nn.Module):
    ...
    def __init__(self, generator, tgt_vocab,
                 ...
                 coref_confscore=1,
                 eps=1e-20,
                 lambda_coverage=1, lambda_coverage2=1):
        ...
        self.criterion = CopyGeneratorCriterion(len(tgt_vocab), force_copy, # just NLLLoss
                                                self.padding_idx)
        ...
        self.lambda_coverage2 = lambda_coverage2
    ...
    def compute_loss(self, batch, output, attns, normalization):
        ...
        scores, coref_scores = self.generator(self._bottle(output),
                                ...
                                batch.map)
        loss = self.criterion(scores, align, target) # just NLLLoss

        # loss for coverage
        #if self.lambda_coverage != 0.0:
        #    coverage_loss = self._compute_coverage_loss(attns.get("std"), attns.get("coverage"))
        #    loss += coverage_loss
        #print(attns.get("coverage").size())
        #print(attns.get("std").size())
        ...
        if self.coref_vocab:
            # calculate coref vocab loss: (output word probability of its pronominal reference word) p_coref ∈ P_v
        ...
        if self.coref_attn:
            # calculate coref attention loss: (conversation attention of coreferent mentions) (β_j)^c
        ...
        if self.flow_history:
            ...
            #print("flow_history")
        if self.lambda_coverage != 0.0:
            coverage_loss = self._compute_coverage_loss(attns.get("p_cov"), attns.get("coverage"))
            loss += coverage_loss
            #print("coverage")
        
        if self.lambda_coverage2 != 0.0:
            coverage_loss2 = self._compute_coverage_loss2(attns.get("qa_cov"), attns.get("coverage2"))
            loss += coverage_loss2
    ...
    '''
    #TODO
    def _compute_coverage_loss(self, passage_attn, coverage_attn1):
        covloss1 = torch.min(passage_attn, coverage_attn1).sum()
        covloss1 *= self.lambda_coverage
        return covloss1
    
    def _compute_coverage_loss2(self, qa_attn, coverage_attn2):
        covloss2 = torch.min(qa_attn, coverage_attn2).sum()
        covloss2 *= self.lambda_coverage2
        return covloss2
    '''
    ...
    def js_div(self, p_output, q_output):
        ...
        #if self.get_softmax:
        p_output = torch.nn.functional.softmax(p_output, dim=1)
        q_output = torch.nn.functional.softmax(q_output, dim=1)
        log_mean_output = ((p_output + q_output)/2).log()
        return (KLDivLoss(log_mean_output, p_output) + KLDivLoss(log_mean_output, q_output))/2

    def _compute_coverage_loss(self, passage_attn, coverage_attn1):
        covloss1 = self.js_div(coverage_attn1, passage_attn)
        covloss1 *= self.lambda_coverage
        return covloss1
    
    def _compute_coverage_loss2(self, qa_attn, coverage_attn2):
        covloss2 = self.js_div(coverage_attn2, qa_attn)
        covloss2 *= self.lambda_coverage2
        return covloss2
